...
contract Autolido {
    ...
    function _deposit(address owner, uint value) private {
    ...
    function amountToBorrow(uint depositAmount) public view returns(uint) {
        (,uint collateralFactorMantissa,) = collateralCToken.comptroller().markets(address(collateralCToken));
        ...
        uint depositTokenPrice = collateralCToken.comptroller().oracle().getUnderlyingPrice(address(collateralCToken)); // Denominated in 36-(Decimals of Underlying) decimals
        uint depositUSDvalue = (depositAmount*depositTokenPrice)/10**(36-collateralToken.decimals()); // Same decimals as underlying

        uint maxBorrowUSD  = (depositUSDvalue*collateralFactorMantissa)/10**18;
        uint safeBorrowUSD = (maxBorrowUSD*safteyMargin)/10**collateralToken.decimals(); // 80% of max borrow by default

        uint borrowTokenPrice = borrowCToken.comptroller().oracle().getUnderlyingPrice(address(borrowCToken)); // Denominated in 36-(Decimals of Underlying) decimals
        return (safeBorrowUSD*10**(2*borrowToken.decimals()-collateralToken.decimals()))/borrowTokenPrice; // Denominated in decimals of borrow token
    }
    ...
    function enterMarket() public onlyOwner returns (uint[] memory) {
    ...
    // !! DEV FUNCTION - MAY BE REMOVED LATER !!
    function exitMarket() public onlyOwner returns (uint) {
        require(collateralCToken.comptroller().exitMarket(address(collateralCToken))==0, "Exit Market Failed");
    }
